...
####################### МЕНЯЕМ ПЕРЕМЕННЫЕ В КАВЫЧКАХ НА АКТУАЛЬНЫЕ ############################
# Логин и пароль для сетевых каталогов будут запрошены в процессе работы скрипта
###############################################################################################
share_ip="192.168.122.95" # ip адрес сетевого каталога
share_dir="/share_win"    # Полный путь/название каталога например /test/docs или /docs/документы 2018

# Для второго сетевого каталога изменить переменные ниже и раскомментировать в основном блоке share2
share_ip2="192.168.122.95" # внутри кавычек меняем на актуальное
share_dir2="/тест тест"    # внутри кавычек меняем на актуальное
###############################################################################################
...
function install() {
  ...
  # Для КриптоПро тип подписи по умолчанию sig
  su - $user -c "/opt/cprocsp/sbin/amd64/cpconfig -ini '\local\cptools\create_sign' -add string signature_extension sig"
  # Для КриптоПро отсоединенная подпись по умолчанию
  su - $user -c "/opt/cprocsp/sbin/amd64/cpconfig -ini '\local\cptools\create_sign' -add long is_detached 1"
...
function host_name() {
  if [[ $(hostname) == arm-* ]]; then
    echo ""
    echo -e "${green} Hostname ${red}$(hostname)${green} , уже существует , если нужно поменять - ручками в системе ${red} =) ${nc}"
    echo ""
  else
    # Меняем hostname , формат "arm-номер_dst"
    while true; do
      echo ""
      ...
      if [[ $HNAME =~ ^(300|[1-2]?[0-9]?[0-9])$ ]]; then
        ...
        echo -e "${red}число от 1 до 100${nc}"
      fi
    done
    hostnamectl set-hostname arm-$HNAME
  fi
}
##############################################################################################
...
function share() {
  ...
  while true; do
    ...
    echo ""
    ping -c 2 $share_ip &>/dev/null
    if [ ! $? -eq 0 ]; then
      echo ""
      echo -e "${red}  IP адрес ${nc}$share_ip${red} для каталога ${nc}$(basename "${share_dir}")${red} не пингуется, проверить IP адрес или соединение!${nc}"
      echo ""
      break
    fi

    read -r -p "Введите логин ( или 'ok' чтобы пропустить подключение ) : " login_share
    # Проверка на команду продолжить
    if [ "$login_share" == "ok" ]; then
      echo -e "${red}Операция прервана пользователем${nc}"
      break
    fi
    read -r -p "Введите пароль : " pass_share
    echo ""

    # Создаем файл /root/.creds с логином и паролем
    echo "username=$login_share" >/root/.creds
    echo "password=$pass_share" >>/root/.creds
    chmod 600 /root/.creds

    # Попытка монтирования с указанным логином и паролем
    mounted=false
    for ver in "${versions[@]}"; do
      if timeout 5 mount -t cifs -o vers=$ver,cred=/root/.creds //"$share_ip""$share_dir" "$mount_point" >/dev/null 2>&1; then
        echo ""
        ...
        echo "share1	-fstype=cifs,rw,user,vers="$ver",noperm,cred=/root/.creds	://"${share_ip}""${share_dir}"" >/etc/auto.tab
...
function share2() {
  ...
  while true; do
    echo "Проверка соединения..."
    echo ""
    ping -c 2 $share_ip2 &>/dev/null
    if [ ! $? -eq 0 ]; then
      echo ""
      echo -e "${red}  IP адрес ${nc}$share_ip2${red} для каталога ${nc}$(basename "${share_dir2}")${red} не пингуется, проверить IP адрес или соединение!${nc}"
      echo ""
      break
    fi

    read -r -p "Введите логин (или 'ok' чтобы пропустить подключение ) : " login_share2
    # Проверка на команду продолжить
    if [ "$login_share2" == "ok" ]; then
      echo -e "${red}Операция прервана пользователем${nc}"
      break
    fi
    read -r -p "Введите пароль : " pass_share2
    echo ""

    # Создаем файл /root/.creds с логином и паролем
    echo "username=$login_share2" >/root/.creds2
    echo "password=$pass_share2" >>/root/.creds2
    chmod 600 /root/.creds2

    # Попытка монтирования с указанным логином и паролем
    mounted=false
    for ver2 in "${versions[@]}"; do
      if timeout 5 mount -t cifs -o vers=$ver2,cred=/root/.creds2 //"$share_ip2""$share_dir2" "$mount_point" >>/dev/null 2>&1; then
        echo ""
        ...
        # Добавляем параметры монтирования в файл /etc/auto.tab
        echo "share2	-fstype=cifs,rw,user,vers="$ver2",noperm,cred=/root/.creds2	://"${share_ip2}""${share_dir2}"" >>/etc/auto.tab
        chmod 644 /etc/auto.tab
    ...
    if [ "$mounted" = true ]; then
      ...
      echo ""
      echo -e "${red} Неудачная попытка монтирования ${nc}$(basename "${share_dir2}")${red} , проверить логин или пароль ${nc}"
      echo ""
    fi
...
function print_hp() {
  subnet=$(echo $share_ip | awk -F '.' '{print $1"."$2"."$3}')
  while true; do
    echo ""
...
EOF
  ...
  # Выключаем автозапуск krb5-ticket-watcher
  cat <<'EOF' >>/etc/xdg/autostart/krb5-ticket-watcher.desktop
Hidden=true
EOF
